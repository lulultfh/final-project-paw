...
import {
  ...
  formatCurrency,
  getCategoryColor,
} from "@/core/hooks/useAnimatedCounter";
...
const HomePage = () => {
  ...
  const [adminName, setAdminName] = useState(""); // Mengubah nilai default menjadi string kosong
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Cari pengguna dengan role 'admin' untuk mendapatkan nama admin
        const adminUser = userData.find((user) => user.role === "admin");

        if (adminUser) {
          setAdminName(adminUser.nama);
        } else if (userData.length > 0) {
          // Fallback: Jika tidak ada user dengan role 'admin', gunakan nama pengguna pertama
          setAdminName(userData[0].nama);
        }
        ...
        const processOrders = ordersData.filter(
        ...
        const totalSoldProducts = productsData.reduce(
        ...
        const productStatistics = Object.entries(categoryCounts).map(
          ([name, value]) => ({
        ...
        setAdminName("Guest"); // Fallback jika fetch gagal
      } finally {
        setIsLoading(false); // Selesai loading, terlepas dari hasil
      }
    };
  ...
  return (
    <div className="min-h-screen p-6">
      <div className="max-w-7xl mx-auto">
        {/* Header */}
        <div className="mb-8">
          <h1 className="font-second italic text-5xl text-[#878B5A] mb-2">
            Admin Dashboard
          </h1>
          <p className="text-gray-600">Lacak performa dan penjualan bakerymu</p>
        </div>

        {/* New Admin Greeting Card */}
        <div className="w-full bg-[#E5E6D7] rounded-lg p-6 mb-8 flex items-center shadow-md">
          <h2 className="text-2xl font-semibold text-[#878B5A]">
            {isLoading ? "Halo, Loading..." : `Halo, ${adminName}! 👋`}
          </h2>
        </div>
