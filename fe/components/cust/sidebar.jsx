...
export default function SidebarLayout() {
  ...
  const mainNav = [
    ...
    { label: "Transaction", icon: "./transaction.svg", path: "/" },
  ];
              ...
              <nav className="flex-1 space-y-1">
                <ul className="space-y-[30px]">
                  {mainNav.map((item) => (
                    <li key={item.label}>
                      <Link href={item.path}>
                        <div className="inline-flex items-center w-full px-4 py-2 mt-1 text-base text-gray-900 transition duration-300 transform rounded-lg bg-transparant hover:bg-[#B6BB79] hover:text-black">
                          <Image
                            src={item.icon}
                            alt={`${item.label} Icon`}
                            width={20}
